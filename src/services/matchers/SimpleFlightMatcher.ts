...
export class SimpleFlightMatcher {
  ...
  public findMatches(bookings: BookingData[], expenses: ExpenseData[]): MatchResult[] {
    ...
    flightBookings.forEach((booking, bookingIndex) => {
      const bookingId = booking.id || `booking-${bookingIndex}`;
      ...
      if (!booking.cardLast4Normalized || booking.cardLast4Normalized === '[No card last 4 found]') {
        console.log(`[SimpleFlightMatcher] Booking ${bookingId} skipped - no card last 4`);
        return;
      }
      ...
      if (bestMatch && bestMatch.score >= MINIMUM_CONFIDENCE) {
        ...
        console.log(`[SimpleFlightMatcher] Matched booking ${bookingId} to expense ${bestMatch.expenseId} with confidence ${bestMatch.score.toFixed(2)}`);
      } else {
        console.log(`[SimpleFlightMatcher] No match found for booking ${bookingId}`);
      }
    });
